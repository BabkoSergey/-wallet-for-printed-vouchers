            ...
            <div class="card-title">
                <div class="row">
                    ...
                    <div class="col-md-6 -text-right">
                        @if(Auth::user()->hasPermissionTo('reseller-user-create'))
                        <a class="btn btn-success  btn-sm" role="button" href="{{ url('wallet/users/create') }}"> {{ __('Create New User') }}</a>                        
                        @endif
                    </div>
...
@if(Auth::user()->hasPermissionTo('reseller-user-delete'))
<div style="display: none">
    <form id="jq_user-delete-form" method="POST" action="" data-url="{{ url('/wallet/users/') }}" accept-charset="UTF-8">
        @csrf
            ...
            columns: [
                ...
                {   data: 'actions',
                    ...
                    render: function ( data, type, row ) {
                        ...
                        actions += '<a href="{{ url('wallet/users') }}/'+row.id+'" class="btn btn-success"><i class="icon-eye icons"></i></a>';
                        actions += '</div>';
                        
                        @if(Auth::user()->hasPermissionTo('reseller-user-edit'))
                        actions += '<div class="btn-group btn-group-sm" style="margin:0 5px;" role="group" aria-label="Basic example">';
                        actions += '<a href="{{ url('wallet/users') }}/'+row.id+'/edit" class="btn btn-primary"><i class="icon-pencil icons"></i></a>';
                        actions += '</div>';
                        @endif
